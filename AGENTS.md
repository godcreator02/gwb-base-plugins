...
plugin-manager(core)；另有已退役的 command-http（包标 private、不发版，入口由 mcp 承担，
判据见轨迹卡「command-http 退役」）。**这个仓的插件零 UI 依赖**：peer 与产物里不许出现 shell /
...
（`readme.mdx` / `agents.mdx`），改完在 `docs-site` 跑 `pnpm exec docfirst render`
