...
在役九个：commands、data、settings、http、skills、node-cli、py-cli、logger(core)、
plugin-manager(core)；另有搁置中的 mcp（入口由 http 接班、default 已卸载，判据
...
2. **命令描述必须写参数形状**（`{ entryId }` 这种；无参数写「无参数」）——经 HTTP 入口的
...
* 插件跑在 Electron 主进程里，`import('electron')` 够得着

## 命名与依赖

* 包名 `@godcreator02/gwb-<插件名>`；服务名一律 `gwb` 开头小驼峰，默认跟插件名对应
