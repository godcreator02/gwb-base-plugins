...
import { describePeers } from '../../src/client/panes/plugin-manager/peers.js'
...
describe('装机回执里 peer 那一行', () => {
  ...
  it('装上的点名，本来就在与跳过的只报数', () => {
    const view = describePeers({
      peers: [
        { pkg: '@godcreator02/docfirst-workflow', range: '>=0.4.0', action: 'installed' },
        { pkg: 'cordis', range: '^4', action: 'skipped' },
        { pkg: '@godcreator02/gwb-commands', range: '>=0.2.0', action: 'present' },
      ],
    })
    expect(view.ok).toBe(true)
    expect(view.line).toBe('\nPeer：装了 @godcreator02/docfirst-workflow；2 条本来就在或用不着装')
  })
