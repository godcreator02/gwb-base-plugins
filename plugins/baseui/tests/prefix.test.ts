 ...
 * **忘加前缀的守卫。**
 *
 * 这个件的样式围栏是「类名自带 `baseui:` 前缀」（见 `src/client/styles.css`）。围栏的代价
 ...
 * **不以 `baseui:` 开头**的 token，拿件自己那份 `styles.css`（**去掉 `prefix(baseui)`**）
 ...
 *
 * **这个件字符串多得反常**——命令名（`plugin-manager.list` 这一串）、包名、条目态、
 * 筛选档、组件的变体名全是字面量。绝大多数自然编不出规则，不用特判；真编得出来的
 * 只有下面那一个。
 */
 ...
 *
 * 别的字面量都编不出规则，一个都不用排：命令名（`plugin-manager.install`）与包名
 * （`@godcreator02/gwb-plugin-manager`）带点带斜杠、`plugin-manager` / `market` /
 * `secret` / `disabled-running` 这些态名与 `all` / `on` / `off` / `bad` 这些筛选档
 * 都不是工具类，编不出东西来。**没编出规则的 token 本来就不该进排除表**——排除表
 * 越短，漏报面越小。
 */
 ...
 *
 * **必须认得注释**，不能只拿正则去捞引号：这些文件的头注里全是反引号代码段，
 * 一对反引号在正则眼里就是一个模板串，注释里随便一个工具类名都会被当成漏加
 * 前缀的类报出来。所以这儿走一遍最小的状态机：行注释、块注释、单双引号、
 * 反引号各是一个状态，只有在字符串状态里收字符。
 */
...
describe('忘加前缀的守卫', () => {
  ...
  it('字符串字面量里没有漏掉 baseui: 前缀的类名', async () => {
    const input = fs.readFileSync(path.join(clientDir, 'styles.css'), 'utf8').replace(' prefix(baseui)', '')
    // 那一行真的摘干净了才算数。头注里也提到 prefix(baseui)，所以先把注释剥掉再看
    ...
    for (const file of files) {
      for (const literal of literals(fs.readFileSync(file, 'utf8'))) {
        for (const token of literal.split(/\s+/)) {
          if (token === '' || token.startsWith('baseui:') || NOT_A_CLASS.has(token)) continue
          tokens.add(token)
        }
      }
    }
