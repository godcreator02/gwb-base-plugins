...
describe('patchRuntime', () => {
  it('只补自己那格（command-http），内核那几格原样保留', () => {
    ...
    expect(out['command-http']).toEqual({ port: 2870, url: 'http://127.0.0.1:2870' })
  })

  it('mcp 与 gwb-http 留下的旧格都是认不得的字段——原样保留不碰', () => {
    const text = JSON.stringify({
      pid: 1,
      mcp: { port: 55572, url: 'http://127.0.0.1:55572/mcp' },
      http: { port: 2870, url: 'http://127.0.0.1:2870' },
    })
  ...
  it('重写同一格时新值覆盖旧值', () => {
    ...
    expect(second['command-http']).toEqual({ port: 2, url: 'u2' })
  })
  ...
  it('读不动、半截 JSON、根不是对象，一律当没有——从空对象起', () => {
    for (const bad of [undefined, '{ 半截', '[1,2]', '"str"']) {
      ...
      expect(out).toEqual({ 'command-http': { port: 1, url: 'u' } })
    }
  })
