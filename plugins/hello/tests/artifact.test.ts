 ...
 */
const PRELUDE = /(?:^|[{};])([^{};]*)\{/g
/** 一个类选择器。转义序列（`.hello\:flex` 里那个 `\:`）整体吃掉 */
const CLASS_SELECTOR = /\.(?:\\.|[A-Za-z0-9_-])+/g
/**
 * **唯一放行的非前缀类**：`.dark` 是页面级的亮暗标志，挂在 `<html>` 上、由令牌那张表
 * 与外壳负责。它只以**祖先**的身份出现在 `hello:dark:*` 编出来的
 * `.hello\:dark\:…:is(.dark *)` 里，不是本件元素身上的类
 */
...
describe.skipIf(!built)('产物', () => {
  ...
  it('style.css 里的类选择器一律 .hello\\: 开头——围栏就是前缀本身', () => {
    const bad = classSelectors(fs.readFileSync(path.join(distDir, 'style.css'), 'utf8'))
      .filter((s) => !s.startsWith('.hello\\:') && !DARK_FLAG.has(s))
      .sort()
    expect(bad).toEqual([])
  })
