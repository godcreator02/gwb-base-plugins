 ...
 * **忘加前缀的守卫。**
 *
 * 这个件的样式围栏是「类名自带 `hello:` 前缀」（见 `src/client/styles.css`）。围栏的代价
 ...
 * **不以 `hello:` 开头**的 token，拿件自己那份 `styles.css`（**去掉 `prefix(hello)`**）
 ...
 *
 * cva 的 `"default"`、`data-probe` 的值、import 说明符这些自然编不出规则，不用特判。
 */
...
/**
 * **唯一的排除**：`outline` 出现在 `variant="outline"` 上（`Button` 的变体名），不是类名。
 * 它恰好也是个 Tailwind 工具类（`outline-style: solid`），所以会被误报。
...
describe('忘加前缀的守卫', () => {
  ...
  it('字符串字面量里没有漏掉 hello: 前缀的类名', async () => {
    const input = fs.readFileSync(path.join(clientDir, 'styles.css'), 'utf8').replace(' prefix(hello)', '')
    // 那一行真的摘干净了才算数。头注里也提到 prefix(hello)，所以先把注释剥掉再看
    ...
    for (const file of files) {
      for (const literal of literals(fs.readFileSync(file, 'utf8'))) {
        for (const token of literal.split(/\s+/)) {
          if (token === '' || token.startsWith('hello:') || NOT_A_CLASS.has(token)) continue
          tokens.add(token)
        }
      }
    }
