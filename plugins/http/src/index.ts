...
import { buildSurface, type SkillsSlot } from './surface.js'

/**
 * HTTP 门：把命令面开成本机无状态 JSON 接口（方向永远是 agent → 工作台）。2026-09-13
 * 接 mcp 件的班——mcp 搁置、default 卸载，理由与判据见轨迹卡「HTTP 门接班」。
 *
 * - **两条路由**：`GET /surface` 现拼命令与说明书清单（带 instructions 与自报 home 名）；
 *   `POST /run` body `{"command": 名字, "args": {...}}` 按名调命令。参数形状不在协议层，
 *   在**命令描述**里——命令自己校验
 ...
 *   而下游要的是「说出 home 名就连得上它」。只补自己那格（`http`），内核那几格原样保留
 */

export const name = 'gwb-http'
...
export function apply(ctx: GwbContext): void {
   ...
   * 箭头函数的理由同 runCommand：function 声明会 hoist，TS 不给 `cli` 保留窄化 */
  const surfaceHandler = async (res: ServerResponse): Promise<void> => {
    const commands = cli.list()
    log.info(`agent 现扫了一遍表面（${commands.length} 条命令）`)
    sendJson(res, 200, buildSurface({ home, version, commands, skills: skills?.list() }))
  }
  ...
  ready.then(
    async (p) => {
      ...
      log.info(`HTTP 门就绪（${home} home）：${endpointUrl(p)}`)
    ...
    cli.register(
      {
        name: 'http.info',
        description:
          'HTTP 门的连接信息（url + token + 两条 curl 例句，仅本机使用，勿外传）。无参数',
        plugin: name,
      },
      async () => {
        /**
        ...
        if (live === null) {
          return { ok: false, error: `HTTP 门没在监听（${home} home 要的是 ${choice.port} 口）：${listenError ?? '原因未记下'}` }
        }
        const token = await ensureToken()
        if (token === null) {
