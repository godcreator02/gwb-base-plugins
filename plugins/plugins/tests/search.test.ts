...
describe('isGwbLine：scope 下、gwb- 前缀', () => {
  ...
  it('共享包、词汇表包也算——它们本来就以 gwb- 开头', () => {
    expect(isGwbLine('@godcreator02/gwb-shared-react')).toBe(true)
    expect(isGwbLine('@godcreator02/gwb-plugin-api')).toBe(true)
  })

  it('没带前缀的废弃包、别人的 scope、裸名，都不算', () => {
    expect(isGwbLine('@godcreator02/shell')).toBe(false)
    expect(isGwbLine('@godcreator02/cli')).toBe(false)
  ...
  it('**坏条目只跳过那一条**：缺 name、package 不是对象、整个条目是标量', () => {
    const got = parseSearch(
      response([
        ...
        'not-an-object',
        entry('@godcreator02/gwb-data'),
      ]),
    )
    expect(got).toEqual([{ pkg: '@godcreator02/gwb-data', version: '0.0.1', description: '@godcreator02/gwb-data 的一句话' }])
  })

  it('version / description 缺了或不是字符串就当没有，不挡这一行', () => {
    const got = parseSearch(
      response([{ package: { name: '@godcreator02/gwb-x', version: 7, description: '' } }]),
    )
    expect(got).toEqual([{ pkg: '@godcreator02/gwb-x' }])
  })

  it('**响应不统一**：没有 version 的条目回退到 dist-tags.latest（实测同一份响应里两种并存）', () => {
    const got = parseSearch(
      response([
        { package: { name: '@godcreator02/gwb-y', description: '旧索引的一条', 'dist-tags': { latest: '0.1.4' } } },
      ]),
    )
    expect(got).toEqual([{ pkg: '@godcreator02/gwb-y', version: '0.1.4', description: '旧索引的一条' }])
  })
})
