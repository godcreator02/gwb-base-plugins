 ...
 * **忘加前缀的守卫。**
 *
 * 外壳自己那张表（`dist/style.css`）的围栏是「类名自带 `shell:` 前缀」（见
 * `src/client/styles.css`）。围栏的代价是**漏加前缀完全静默**：`flex` 写成这样，
 * Tailwind 在带前缀的编译下一个字节都不出，页面上就是那一处没样式——不报错、不告警、
 * 构建照样绿。选择器 scope 那套方案下漏写类名是编得出来的，这类错原先不存在，是换方案
 * 换出来的新坑，所以得有一条守卫钉住。
 *
 * 判法：把 `src/client` 下所有 tsx 里的字符串字面量抽出来，按空白切成 token；对每个
 * **不以 `shell:` 开头**的 token，拿外壳自己那份 `styles.css`（**去掉 `prefix(shell)`**）
 ...
 *
 * **只管 `dist/style.css` 这一张。** 另一张 `dist/dockview.css` 由 `build.ts` 拼，管的是
 * `.dv-*` 与 `.dockview-theme-gwb`——那些类名由 dockview 的 JS 写、不经 Tailwind 的手，
 * 跟前缀无关，所以这条守卫扫的是 tsx 而不是 css。
 *
 * 命令名（`theme.startupCss`）、dockview 的组件名（`gwb-tab`）、`'right'` 这类方向值、
 * 图标裸名（`flask-conical`）这些自然编不出规则，不用特判。
 */
...
/**
 * **误报的排除项。** 空集就是这个件此刻的真相：外壳的字符串字面量里虽然有一堆命令名、
 * dockview 的组件名与主题类名、`'right'` 这类方向值，但它们在不带前缀的编译器下**一个
 * 都编不出规则**，不构成误报。真撞上再往这儿加，并且每加一条得写清「它为什么不是类名」。
 */
...
describe('忘加前缀的守卫', () => {
  ...
  it('字符串字面量里没有漏掉 shell: 前缀的类名', async () => {
    const input = fs.readFileSync(path.join(clientDir, 'styles.css'), 'utf8').replace(' prefix(shell)', '')
    // 那一行真的摘干净了才算数。头注里也提到 prefix(shell)，所以先把注释剥掉再看
    ...
    for (const file of files) {
      for (const literal of literals(fs.readFileSync(file, 'utf8'))) {
        for (const token of literal.split(/\s+/)) {
          if (token === '' || token.startsWith('shell:') || NOT_A_CLASS.has(token)) continue
          tokens.add(token)
        }
      }
    }
