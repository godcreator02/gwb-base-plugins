...
/**
 * 件的数据落盘。`ctx.gwbStore` 交出来的那一格**自动绑定到取它的那个件**——
...
/** 消费方拿到的那一格。写 `inject: ['gwbStore']` 才有 */
export interface GwbStoreApi {
...
export default class GwbStore extends Service implements GwbStoreApi {
  ...
  private owner(): string {
    ...
    if (typeof name !== 'string' || name === '') {
      throw new Error('取不到调用方的身份——ctx.gwbStore 只能在经 cordis.yml 挂上的件里用')
    }
    return name
  }
